...
from services.existing_management.clickable_tile import ClickableTile, ClickableTileSimple
...
def create_tile(parent, entity: Entity, config: TileConfig) -> ClickableTile:
    ...
    tile.btn_left.setIconSize(QSize(24, 24))


    tile.btn_right.setIconSize(QSize(24, 24))

    tile.btn_edit.setIconSize(QSize(24, 24))

    tile.btn_delete.setIconSize(QSize(24, 24))
...
def create_tile_simple(parent, entity: Entity) -> ClickableTileSimple:
    tile = ClickableTileSimple(parent)
    tile.setProperty("entity_id", entity.get_id())
    tile.setProperty("entity", entity)
    tile.setMinimumSize(QSize(250, 200))
    tile.setMaximumSize(QSize(250, 200))
...
def insert_tile_simple(grid_widget, entity: Entity, tile_width: int = 250, spacing: int = 10):
    tile = create_tile_simple(grid_widget, entity)
    cols = max(1, grid_widget.width() // (tile_width + spacing))
    count = grid_widget.layout().count()
    grid_widget.layout().addWidget(tile, count // cols, count % cols)
    return tile
